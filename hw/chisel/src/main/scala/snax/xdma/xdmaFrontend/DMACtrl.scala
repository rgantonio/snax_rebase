...
class DMACtrlIO(param: DMACtrlParam) extends Bundle {
    ...
    val reader_ext_cfg_o = if (param.readerextparam.length != 0) {
        Output(
          Vec(
            param.readerextparam.map { i => i.userCsrNum }.reduce(_ + _) + param.readerextparam.length,
            UInt(32.W)
          )
        ) // Buffered within Extension Base Module
    } else Input(Vec(0, UInt(32.W)))

    val writer_ext_cfg_i = if (param.writerextparam.length != 0) {
        Input(
          Vec(
            param.writerextparam.map { i => i.userCsrNum }.reduce(_ + _) + param.writerextparam.length, 
            UInt(32.W)
          )
        ) // Buffered within Extension Base Module
    } else Input(Vec(0, UInt(32.W)))
