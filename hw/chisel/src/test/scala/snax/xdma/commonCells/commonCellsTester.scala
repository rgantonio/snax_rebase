...
class complexQueue_Tester extends AnyFlatSpec with ChiselScalatestTester {
    "The test of complexQueue (64->512)" should " pass" in {
        test(new complexQueue_Concat(64, 512, 16)).withAnnotations(Seq(WriteVcdAnnotation)) {dut =>
            ...
            for (i <- 0 until 16) {
                dut.io.out(0).bits.expect((BigInt(i*8+7)<<(64*7))+(BigInt(i*8+6)<<(64*6))+(BigInt(i*8+5)<<(64*5))+(BigInt(i*8+4)<<(64*4))+(BigInt(i*8+3)<<(64*3))+(BigInt(i*8+2)<<(64*2))+(BigInt(i*8+1)<<(64*1))+(BigInt(i*8+0)<<(64*0)))
    ...
    "The test of complexQueue (512->64)" should " pass" in {
        test(new complexQueue_Concat(512, 64, 16)).withAnnotations(Seq(WriteVcdAnnotation)) {dut =>
            ...
            //Writing data
            for (i <- 0 until 16) {
                dut.io.in(0).bits.poke((BigInt(i*8+7)<<(64*7))+(BigInt(i*8+6)<<(64*6))+(BigInt(i*8+5)<<(64*5))+(BigInt(i*8+4)<<(64*4))+(BigInt(i*8+3)<<(64*3))+(BigInt(i*8+2)<<(64*2))+(BigInt(i*8+1)<<(64*1))+(BigInt(i*8+0)<<(64*0)))
